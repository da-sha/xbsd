...
DROP PROCEDURE IF EXISTS `tsmsv3`.`arrange_academy_select_course`$$

CREATE DEFINER=`root`@`%` PROCEDURE `arrange_academy_select_course`(t_year INT,quarter INT)
    ...
    BEGIN
		...
		declare fetchSeqCursor cursor for
			SELECT 	course.courseid as courseid,class.classid as classid,course.name as coursename
			FROM 	course,class
			WHERE   course.cou_type != 1			/*不是必修课*/
				and course.cou_category = 2		/*专业平台课*/
