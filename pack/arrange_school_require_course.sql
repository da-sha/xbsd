...
DROP PROCEDURE IF EXISTS `tsmsv3`.`arrange_school_require_course`$$

CREATE DEFINER=`root`@`%` PROCEDURE `arrange_school_require_course`(t_year INT,quarter INT)
BEGIN
		...
		declare fetchSeqCursor cursor for
			SELECT 	course.courseid as courseid,class.classid as classid
			FROM 	course,class,major
			WHERE   course.cou_type = 1			/*必修课*/
				and course.cou_category = 3		/*专业平台课*/
				and course.level = 1			/*本科生*/
				and class.majorid = major.majorid and major.level = 1
