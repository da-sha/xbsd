...
DROP PROCEDURE IF EXISTS `tsmsv3`.`arrange_academy_require_course`$$

CREATE DEFINER=`root`@`%` PROCEDURE `arrange_academy_require_course`(t_year INT,quarter INT)
BEGIN
		...
		declare fetchSeqCursor cursor for
			...
			WHERE   course.cou_type = 1			/*必修课*/
				and course.cou_category = 2		/*学院平台课*/
				...
				WHILE fetchSeqOk DO
					...
					IF b_inserted = false THEN
						...
						INSERT INTO teach_task(courseid,teach_year,teach_quarter,status,finish_status)
							values (_courseid,t_year,quarter,1,3);		/*授课状态为待审，拟授*/
						/*向teach_class表中插入数据*/
