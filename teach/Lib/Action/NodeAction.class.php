...
class NodeAction extends CommonAction {
	...
	function index(){
		...
		$str  = "<tr>
					<td align='center'>
						<input name='listorders[\$id]' type='text' size='2' value='\$seq' class='input-text-c'>
					</td>
					<td >\$spacer\$title</td>
					<td >\$name</td>
		...
		foreach($this->groups as $key=>$res){
			...
			foreach($result as $r) {
				$r['str_manage'] = '<a href="'.
									U('Node/add',array( 'pid' => $r['id'],'groupid'=>$r['groupid'])).
									'">'.
									L('add').
									'</a> | <a href="'.
									U('Node/edit',array( 'id' => $r['id'])).
									'">'.
									L('edit').
									'</a> | <a href="javascript:confirm_delete(\''.
									U('Node/delete',array( 'id' => $r['id'])).
									'\',\''.
									L('confirm',array('message'=>$r['cname'])).
									'\')">'.
									L('delete').
									'</a> ';
